"""
策略: KDJ + PVT + 斐波那契回撤 量价组合 (KDJ + Price Volume Trend + Fibonacci Retracement)
------------------------------------------------------
信号逻辑:
  进场 (1): K < kdj_oversold 且 D < kdj_oversold 且 J < kdj_oversold
            且 PVT > PVT移动均线
            且 Close <= Fib_0.618支撑位  （KDJ三线全超卖 + 量价趋势上升 + 处于斐波那契强支撑区）
  离场 (0): K > kdj_overbought 或 J > kdj_overbought
            或 PVT < PVT移动均线
            或 Close >= Fib_0.382阻力位  （KDJ超买 或 量价趋势走弱 或 价格触及斐波那契阻力）
  中间状态: 维持上一持仓
...
超参数:
  ...
  pvt_ma_period   (default 20)   PVT 平滑移动平均周期
...
PVT 计算:
  ...
  PVT 对价格涨跌幅度加权，与 KDJ 配合可更精准反映量价背离情况
...
def run(data: pd.DataFrame, config: dict):
    if "Volume" not in data.columns:
        raise ValueError("KDJ+PVT 策略需要 'Volume' 列，当前数据中未找到。")
    ...
    position  = 0
    positions = []
    for k, d, j, pvt_val, pvt_ma_val, close_val, f618, f382 in zip(
        df["K"], df["D"], df["J"], df["pvt"], df["pvt_ma"], df["Close"], df["fib_0618"], df["fib_0382"]
    ):
        if position == 0:
            # KDJ 三线全超卖 + PVT 上升确认 + 价格处于斐波那契 0.618 支撑区 → 进场
            if k < oversold and d < oversold and j < oversold and pvt_val > pvt_ma_val and close_val <= f618:
                position = 1
        else:
            # KDJ 超买（K 或 J 任一超买）或 PVT 走弱 或 价格触及斐波那契 0.382 阻力位 → 离场
            if k > overbought or j > overbought or pvt_val < pvt_ma_val or close_val >= f382:
                position = 0
