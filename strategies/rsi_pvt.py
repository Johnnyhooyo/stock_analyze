"""
策略: RSI + PVT + 斐波那契回撤 量价组合 (RSI + Price Volume Trend + Fibonacci Retracement)
------------------------------------------------------
信号逻辑:
  进场 (1): RSI < rsi_oversold  且  PVT > PVT移动均线
            且  Close <= Fib_0.618支撑位  （超卖+量价趋势上升+处于斐波那契强支撑区）
  离场 (0): RSI > rsi_overbought  或  PVT < PVT移动均线
            或  Close >= Fib_0.382阻力位  （超买 或 量价趋势走弱 或 价格触及斐波那契阻力）
  中间状态: 维持上一持仓
...
超参数:
  ...
  pvt_ma_period   (default 20)   PVT 平滑移动平均周期（用于判断 PVT 趋势方向）
  fib_period      (default 60)   斐波那契回撤计算窗口（取最近 N 根K线的最高/最低价）

PVT 计算:
  PVT = cumsum( (Close - Close.shift(1)) / Close.shift(1) * Volume )
  PVT 与 OBV 的区别: PVT 按价格变化幅度加权成交量，对价格涨跌幅度更敏感
  PVT 上穿均线 → 量价合力上升，确认做多信号
  PVT 下穿均线 → 量价合力减弱，触发离场
...
def run(data: pd.DataFrame, config: dict):
    if "Volume" not in data.columns:
        raise ValueError("RSI+PVT 策略需要 'Volume' 列，当前数据中未找到。")
    ...
    pvt_ma_period = int(config.get("pvt_ma_period", 20))
    ...
    df = df.dropna(subset=["rsi", "pvt_ma", "fib_0618", "fib_0382"])

    position  = 0
    positions = []
    for rsi_val, pvt_val, pvt_ma_val, close_val, f618, f382 in zip(
        df["rsi"], df["pvt"], df["pvt_ma"], df["Close"], df["fib_0618"], df["fib_0382"]
    ):
        if position == 0:
            # 超卖 + PVT 上升确认 + 价格处于斐波那契 0.618 支撑区 → 进场
            if rsi_val < oversold and pvt_val > pvt_ma_val and close_val <= f618:
                position = 1
        else:
            # 超买 或 PVT 走弱 或 价格触及斐波那契 0.382 阻力位 → 离场
            if rsi_val > overbought or pvt_val < pvt_ma_val or close_val >= f382:
                position = 0
