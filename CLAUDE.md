...
4. **Save Factors** — Persist best factor to `data/factors/`
5. **Training Notify** — Push training summary (Sharpe / validated) to Feishu

> 推断 / 操作建议由 `daily_run.py` 负责，不在 `main.py` 中执行。

### Key Modules

| Module | Purpose |
|--------|---------|
| `main.py` | Entry point, orchestrates full pipeline (delegates to `pipeline/`) |
| `analyze_factor.py` | Core backtesting engine, strategy discovery, hyperparam search |
| `pipeline/` | 训练管道子包：`data_prep` (数据就绪) / `train` (超参搜索) / `select` (因子保存) / `train_portfolio` (组合训练 + meta-model) |
...
├── meta_aggregator.py   # Stacking 第二层 meta-learner（Logistic Regression 融合基础策略信号）
├── portfolio_risk.py    # 组合级风控（仓位上限/行业集中度/相关性/VaR/组合止损）
├── stock_screener.py    # 全市场量化选股（动量 + 趋势 + 量价多维评分）
├── screener_factors.py  # 选股因子计算
└── screener_backtest.py # 选股回测
```
...
**集成状态（截至 2026-04-07）**：注册表集成已完整。`signal_aggregator.py` 默认 `use_registry=True`，`_filter_by_registry()` 支持大小写不敏感匹配，并在过滤后为空时自动 fallback 到全部磁盘因子（防止注册表与磁盘不同步时误过滤）。`main.py:_save_factor()` 在保存因子时同步调用 `registry.register()`，subdir 命名统一为 `ticker.replace(".", "_").upper()`。端到端集成测试见 `tests/test_integration.py::TestFactorRegistryAggregateIntegration`。
...
**Walk-Forward Pruning** (`config.yaml → use_wf_pruning`, default `false`): Optuna trials can be early-pruned if Walk-Forward window win rate falls below `wf_min_window_win_rate`. This avoids wasting compute on strategies that fail out-of-sample.
...
```bash
pytest tests/ -v          # 全离线，当前 284 个测试
pytest tests/test_factor_registry.py -v  # 仅跑单个文件
...
| `tests/test_position_manager.py` | 18 | Kelly 边界、熔断触发、ATR 止损、冷却期 |
| `tests/test_backtest.py` | 10 | 必填字段、全持/全空信号、ATR 止损、前视偏差修复、向量化引擎 |
| `tests/test_strategies.py` | 5 | 策略基础测试 |
| `tests/test_data_manager.py` | 5 | schemas 集成、原子 CSV 写入 |
| `tests/test_signal_aggregator.py` | 20 | 空目录降级、置信度边界、`AggregatedSignal` 属性、meta-aggregator 集成 |
| `tests/test_integration.py` | 13 | 策略→回测全流程、round-trip、因子注册表端到端集成 |
| `tests/test_factor_registry.py` | 24 | TTL 边界、Sharpe 劣化、归档、迁移脚本 |
| `tests/test_meta_aggregator.py` | 13 | MetaAggregator 训练/推断/保存/加载 |
| `tests/test_portfolio_risk.py` | 18 | 组合级风控：仓位上限、行业集中度、VaR、去杠杆 |
| `tests/test_pnl_tracker.py` | 33 | PnL 历史追踪、日收益计算、报告生成 |
| `tests/test_stock_screener.py` | 25 | 选股引擎评分、排序、过滤 |
| `tests/test_screener_backtest.py` | 7 | 选股回测 |
| `tests/test_rnn_strategy.py` | 14 | GRU/LSTM 策略训练/推断 |
| `tests/test_portfolio_training.py` | 6 | 组合训练流程 |
| `tests/test_wf_pruning.py` | 10 | Walk-Forward 剪枝 |

> **全程离线**，无任何网络请求（284 tests）。
...
| # | 任务 | 优先级 | 状态 | 关键影响文件 |
|---|------|--------|------|------------|
| 1 | Ensemble / Stacking 策略 | P1 | ✅ 完成 | `engine/meta_aggregator.py`（Logistic Regression stacking），`signal_aggregator.py` 集成 |
| 2 | Portfolio-Level 风控 | P1 | ✅ 完成 | `engine/portfolio_risk.py`（仓位/行业/相关性/VaR/组合止损） |
| 3 | 因子生命周期管理 | P1 | ✅ 完成 | `data/factor_registry.py` + `signal_aggregator.py` + `main.py:_save_factor()` 集成完整，端到端测试通过 |
...
| 7 | main.py 职责收敛 | P1 | ✅ 完成 | `main.py` 移除预测步骤，仅保留训练 + 飞书摘要；`generate_signal_report` 暂停调用 |
